    ...
    <div className="flex min-h-screen bg-slate-950 text-slate-100"> {/* Dark background, light text */}
      ...
      <nav className="w-64 bg-slate-900 shadow-2xl flex-shrink-0 border-r border-slate-800"> {/* Darker sidebar, strong shadow, subtle border */}
        ...
        <div className="p-6 mt-auto">
          <button onClick={logout} className="w-full flex items-center justify-center space-x-2 bg-slate-800 hover:bg-slate-700/80 border border-slate-700 hover:border-red-500 text-red-400 px-4 py-2 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105">
            <Settings size={20} />
            <span>Logout</span>
          </button>
        </div>
