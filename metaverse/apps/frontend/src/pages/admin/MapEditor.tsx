import React, { useState, useRef, useEffect, type FC } from 'react';
  ...
  import { Layers, Ruler, Download, Eye, Trash2, XCircle, Square, Maximize, Minimize } from 'lucide-react'; // Added Square, Maximize, Minimize for icons
  ...
  const AssetPalette: FC<{ assets: Asset[] }> = ({ assets }) => (
    <div className="w-56 bg-slate-900 border-r border-slate-700 p-4 flex flex-col space-y-4 overflow-y-auto custom-scrollbar shadow-lg">
      ...
      {assets.length === 0 ? (
        <p className="text-slate-500 text-sm text-center py-4 px-2">No assets available. Drag here from another source or add in your asset manager!</p>
      ) : (
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  target.src = "https://placehold.co/80x80/334155/e2e8f0?text=Error"; // Placeholder on error
                }}
              />
              <p className="text-xs text-slate-400 truncate w-full text-center">{a.id.substring(0, 6)}...</p>
            </div>
          ))}
        </div>
      )}
  ...
  }> = ({ data, assets, backgrounds, onClose, isFullScreen, toggleFullScreen }) => {
    const bg = backgrounds.find(b => b.id === data.background);
    const bgImg = useImage(bg?.url ?? '');

    const previewWidth = isFullScreen ? window.innerWidth * 0.9 : Math.min(data.width, 300);
    const previewHeight = isFullScreen ? window.innerHeight * 0.9 : Math.min(data.height, 200);

    return (
      <div className={`flex flex-col space-y-4 p-4 bg-slate-900 rounded-lg border border-slate-700 shadow-xl ${isFullScreen ? 'fixed inset-0 z-50 m-auto max-w-full max-h-full overflow-auto' : 'max-w-sm w-full'}`}>
        ...
        <div className="border border-slate-600 rounded-md overflow-hidden bg-slate-700 flex items-center justify-center" style={{ width: previewWidth, height: previewHeight }}>
          <Stage width={data.width} height={data.height} style={{ backgroundColor: '#2d3748', maxWidth: '100%', maxHeight: '100%' }}>
            <Layer>
        ...
        <Label htmlFor="json-data" className="text-slate-300 text-sm flex items-center gap-2">
  ...
  export const CanvasEditor: FC<{ assets: Asset[]; backgrounds: Background[]; onUpdateCanvas: (data: CanvasJSON) => void }> = ({ assets, backgrounds, onUpdateCanvas }) => {
    ...
    const handleDrop = (e: React.DragEvent) => {
      ...
      if (x >= 0 && y >= 0 && x <= size.width && y <= size.height) {
        ...
        setElements(prev => [...prev, { id: newId, assetId, x, y, width: asset.width, height: asset.height }]);
    ...
    const togglePreview = () => {
      ...
      if (!showPreview && !previewData) { // If toggling on and no data, export it
        exportForPreview();
      }
    };
    ...
    return (
      <div className="flex flex-col lg:flex-row h-[calc(100vh-250px)] bg-slate-900 rounded-lg overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="p-4 border-b border-slate-700 bg-slate-800 flex flex-wrap items-center gap-6 shadow-md">
            <div className="flex items-center gap-2">
              <Label htmlFor="background-select" className="text-slate-300 text-sm flex items-center gap-2 font-medium">
                <Layers className="h-4 w-4 text-purple-400" /> Background:
              </Label>
              <Select value={bgId} onValueChange={setBgId} disabled={backgrounds.length === 0}>
                ...
                <SelectContent className="bg-slate-800 border-slate-700 text-slate-100">
                  {backgrounds.length === 0 ? (
                    <SelectItem value="no-background" disabled className="text-slate-500">No backgrounds available</SelectItem>
                  ) : (
                    backgrounds.map(b => (
                      <SelectItem key={b.id} value={b.id} className="hover:bg-slate-700 focus:bg-slate-700">
                        {b.id.substring(0, 8)}...
                      </SelectItem>
                    ))
                  )}
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="width-input" className="text-slate-300 text-sm flex items-center gap-2 font-medium">
              ...
              <Input
                ...
                type="number"
                value={size.width}
                onChange={e => setSize({ ...size, width: Math.max(1, Number(e.target.value)) })}
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="height-input" className="text-slate-300 text-sm flex items-center gap-2 font-medium">
              ...
              <Input
                ...
                type="number"
                value={size.height}
                onChange={e => setSize({ ...size, height: Math.max(1, Number(e.target.value)) })}
                className="w-24 bg-slate-700 border-slate-600 text-slate-100 focus:border-purple-500"
                min={100}
                placeholder="Height"
              />
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="num-elements" className="text-slate-300 text-sm flex items-center gap-2 font-medium">
                  <Square className="h-4 w-4 text-teal-400" /> Elements:
              </Label>
              <span id="num-elements" className="text-slate-100 font-bold">{elements.length}</span>
            </div>
          ...
          >
            {elements.length === 0 && (
                <div className="absolute inset-0 flex items-center justify-center text-slate-500 text-lg pointer-events-none">
                    Drag assets from the left palette onto the canvas to start designing!
                </div>
            )}
            <Stage
              width={size.width}
        ...
        {showPreview && previewData && (
          <div className={`w-full lg:w-96 bg-slate-900 border-t lg:border-t-0 lg:border-l border-slate-700 p-4 flex flex-col space-y-4 overflow-y-auto custom-scrollbar shadow-lg ${isFullScreen ? 'fixed inset-0 z-50' : ''}`}>
            <PreviewCanvas
              data={previewData}
              assets={assets}
