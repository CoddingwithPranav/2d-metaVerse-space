...
export const AdminDashboard: React.FC = () => {
  ...
  const recentActivities = [
    { id: 1, type: 'User Registered', description: 'New user "John Doe" signed up.', time: '2 hours ago', status: 'success' },
    { id: 2, type: 'Element Added', description: 'New element "Sphere" was created.', time: 'yesterday', status: 'info' },
    { id: 3, type: 'Map Update Failed', description: 'Central region map data sync failed.', time: '3 days ago', status: 'error' },
    { id: 4, type: 'Admin Login', description: 'Admin "Jane Smith" logged in.', time: '5 days ago', status: 'info' },
  ];

  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'success': return <CheckCircle size={16} className="text-green-400 mr-2" />;
      case 'error': return <AlertCircle size={16} className="text-red-400 mr-2" />;
      case 'info':
      default: return <Activity size={16} className="text-blue-400 mr-2" />;
    }
  };
          ...
          <ul className="divide-y divide-slate-700">
            {recentActivities.length > 0 ? (
              recentActivities.map((activity) => (
                <li key={activity.id} className="p-4 flex justify-between items-center hover:bg-slate-700 transition-colors">
                  <div className="flex items-center">
                    {getStatusIcon(activity.status)}
                    <div>
                      <p className="text-lg font-medium text-slate-100">{activity.type}</p>
              ...
              <li className="p-4 text-slate-500 text-center">No recent activity.</li>
            )}
          </ul>
        </div>
      </div>
