...
export const UserSpace: React.FC = () => {
  ...
  const renderSpaceCards = () => {
    if (isLoading) {
      return <p className="col-span-full text-center text-slate-400">Loading your spaces...</p>;
    }
    ...
    return filteredSpaces.map(space => (
        <Card key={space.id} className="bg-slate-800/70 border-slate-700 rounded-xl shadow-lg overflow-hidden flex flex-col">
          <img
            src={space.thumbnail}
            alt={space.name}
          ...
          <CardContent className="p-4 flex flex-col flex-grow">
            <h3 className="text-lg font-semibold text-slate-100 flex-grow">{space.name}</h3>
             {space.map && (
            ...
            <div className="flex gap-2 pt-4 mt-auto">
              <Button variant="outline" className="flex-1" onClick={() => navigate(`/user/spaces/${space.id}/edit`)}>
                Manage
              </Button>
              <Dialog>
                <DialogTrigger asChild>
                  <Button className="flex-1" onClick={() => setSelectedSpace(space)}>
                    Enter
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  ...
                  <DialogDescription>
                    Are you sure you want to enter the space "{selectedSpace?.name}"?
                  </DialogDescription>
                  <DialogFooter className="pt-4">
                    <Button variant="ghost" onClick={() => setSelectedSpace(null)}>Cancel</Button>
  ...
  return (
    <AnimatedPageWrapper id="spaces" className="bg-gradient-to-b from-slate-900 to-slate-950">
      ...
      <p className="text-lg text-slate-400 text-center mb-12 max-w-2xl mx-auto">
        Here are all of the spaces you've created. Manage them or jump right into the action.
      </p>
