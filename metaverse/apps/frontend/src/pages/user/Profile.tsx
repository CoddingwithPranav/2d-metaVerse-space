...
const profileFormSchema = z.object({
  ...
  profileImage: z.string().url("Must be a valid URL.").optional().or(z.literal('')), // Allow empty string for optional
});
  ...
  if (loading && !user) {
    return (
      <AnimatedPageWrapper className="bg-gradient-to-b from-slate-900 to-slate-950 flex items-center justify-center">
        <div className="text-xl text-slate-300 animate-pulse">Loading profile...</div>
      </AnimatedPageWrapper>
    );
  }
  ...
  return (
    <AnimatedPageWrapper id="profile" className="bg-gradient-to-b from-slate-900 to-slate-950">
      ...
      <Card className="max-w-3xl mx-auto bg-slate-800/70 p-6 sm:p-8 rounded-xl shadow-2xl border border-slate-700 transition-all duration-300 hover:shadow-purple-500/30">
        <CardHeader className="text-center pb-6"> {/* Added padding-bottom */}
          ...
          <CardDescription className="text-slate-400 text-base"> {/* Adjusted text color and size */}
            View or update your display name and profile image.
          </CardDescription>
        </CardHeader>

        <CardContent className="flex flex-col items-center gap-8"> {/* Increased gap */}
          <div className="relative group"> {/* Added group for hover effect */}
            ...
            {isEditing && (
              <Dialog open={isUploadDialogOpen} onOpenChange={setUploadDialogOpen}>
                ...
                <DialogContent className="bg-slate-800 text-slate-100 border-slate-700 rounded-lg shadow-xl"> {/* Themed Dialog */}
                  <DialogHeader>
                    <DialogTitle className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-cyan-400 text-2xl font-bold">Upload New Profile Image</DialogTitle>
                    <DialogDescription className="text-slate-400">Select a new image for your profile.</DialogDescription>
                  </DialogHeader>
                  <div className="py-4">
                    <UploadExample 
                      onUpload={url => { 
                    ...
                    />
                     <div className="mt-4 text-sm text-slate-500 flex items-center justify-center gap-2">
                        <UploadCloud size={16} /> Image will be uploaded upon saving changes.
                     </div>
                  </div>
          ...
          {!isEditing ? (
            <div className="w-full text-center space-y-3"> {/* Increased space-y */}
              <p className="text-3xl font-bold text-slate-50">{user.displayName}</p> {/* Bolder, brighter text */}
              <p className="text-lg text-purple-400">@{user.username}</p> {/* More prominent accent */}
              <div className="pt-6 flex flex-col sm:flex-row justify-center gap-4"> {/* Increased pt */}
                <Button 
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 w-full max-w-sm">
                <div>
                  <Label htmlFor="username-display" className="block text-sm font-medium text-slate-300 mb-2">Username</Label> {/* Themed label */}
                  <Input 
                    id="username-display"
                    value={`@${user.username}`} 
                    disabled 
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className='text-slate-300'>Display Name</FormLabel> {/* Themed label */}
                      <FormControl>
                        <Input 
                      ...
                      <FormMessage className="text-red-400" /> {/* Themed message */}
                    </FormItem>
                  )}
                />
                <div className="flex flex-col sm:flex-row gap-4 pt-4"> {/* Increased pt */}
                  <Button 
                    type="submit" 
