  ...
  import { UploadCloud, CheckCircle2, XCircle, FileInput, RotateCw, ImageOff } from 'lucide-react'; // Added ImageOff icon
  ...
  const UploadExample: React.FC<UploadExampleProps> = ({ onUpload, setIsUploadCompleted }) => {
    ...
    const handleUpload = async () => {
      if (!selectedFile) { // Use selectedFile from state
    ...
    return (
      <div className="space-y-4 p-4 rounded-lg border border-slate-700 bg-slate-800 shadow-lg">
        <Label htmlFor="file-upload" className="block text-sm font-medium text-slate-300 mb-2 flex items-center gap-2">
        ...
        {progress > 0 && progress < 100 && !error && (
          <div className="space-y-2 pt-2">
            <Progress value={progress} max={100} className="w-full h-2 bg-slate-700 [&::-webkit-progress-bar]:bg-slate-700 [&::-webkit-progress-value]:bg-gradient-to-r [&::-webkit-progress-value]:from-purple-500 [&::-webkit-progress-value]:to-cyan-500 rounded-full" />
            <p className="text-sm text-slate-400 text-right">{progress.toFixed(0)}%</p>
          </div>
        )}
        ...
        {/* --- LIVE PREVIEW AREA --- */}
        {previewUrl && !isUploading && !localUrl && ( // Show preview if a file is selected and not yet uploaded/failed
          <div className="border border-slate-700 rounded-lg p-4 bg-slate-700 shadow-md flex flex-col items-center text-center">
            ...
            <img src={previewUrl} alt="Selected Preview" className="w-full max-h-64 object-contain rounded-md border border-slate-600 bg-slate-600 p-1" />
            <p className="text-xs text-slate-400 mt-2 break-all">
              File: {selectedFile?.name || 'N/A'} ({selectedFile ? (selectedFile.size / 1024).toFixed(2) : 0} KB)
            </p>
          </div>
        )}

        {/* --- UPLOADED IMAGE AREA --- */}
        {localUrl && !isUploading && !error && ( // Display uploaded image and URL after successful upload
          <div className="border border-slate-700 rounded-lg p-4 bg-slate-700 shadow-md flex flex-col items-center text-center">
            <Label className="text-lg font-semibold text-slate-100 flex items-center gap-2 mb-3">
              <CheckCircle2 className="h-6 w-6 text-green-400" /> Uploaded Image:
            </Label>
            <img src={localUrl} alt="Uploaded" className="w-full max-h-64 object-contain rounded-md border border-slate-600 bg-slate-600 p-1" />
            <p className="text-xs text-slate-400 mt-4 break-all">
              URL: <a href={localUrl} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline transition-colors">{localUrl}</a>
            </p>
          </div>
        )}
      </div>
